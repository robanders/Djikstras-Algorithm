...
public class DiGraph implements DiGraph_Interface {
	...
	public ShortestPathInfo[] shortestPath(String label) {
		...
		while(pathQueue.size() > 0){
			entryPair = pathQueue.getMin();
			pathQueue.delMin();
			vertex = labelMap.get(entryPair.value);
			if(vertex.processed == false){
				...
				while(edgeIterator.hasNext()){
					...
					if(edge.getdestinationVertex().sp > edge.getweight() + edge.getsourceVertex().sp){
						...
						entryPair = new EntryPair(edge.getdestinationVertex().getlabel(), (int) edge.getdestinationVertex().sp);
						pathQueue.insert(entryPair);
					}
				}
			}	
		...
		Iterator<Vertex> vertexIterator = vertexList.iterator();
		ShortestPathInfo shortestPath;
		ShortestPathInfo[] array = new ShortestPathInfo[(int) this.numNodes];
		int index = 0;
		while(vertexIterator.hasNext()){
			vertex = vertexIterator.next();
			//System.out.println("vertex: " + vertex.getlabel() + " " + vertex.sp);
			if(vertex.processed == false){
				vertex.sp = -1;
			}
			shortestPath = new ShortestPathInfo(vertex.getlabel(), vertex.sp);
			array[index] = shortestPath;
			System.out.println(shortestPath.toString());
			index++;
			vertex.processed = false; //resetting for next time
			vertex.sp = 9999999;
			vertex.prevVertex = null;
		}
		return array;
